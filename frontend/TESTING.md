# Open-X-Gest Frontend Testing Guidelines

This document outlines the testing strategy, toolchain configuration, patterns, and conventions for unit and integration testing across the **Open-X-Gest** Angular frontend.
...
it('should inject Authorization Bearer header when token exists', () => {
  ...
  authService.setSession({
    ...
    tokenType: 'Bearer',
    user: { id: 1, name: 'Alex', email: 'alex@open-x-gest.internal', role: 'admin' }
  });
  ...
  describe('authGuard', () => {
    it('should allow access when user is authenticated', () => {
      authService.setSession({
        token: 'active_token',
        tokenType: 'Bearer',
        user: { id: 1, name: 'User', email: 'u@open-x-gest.internal', role: 'admin' }
      });
